...
def check_portfolio_risk(
    ...
    max_forex = limits.get("max_forex_positions", 6)
    max_stock = limits.get("max_stock_positions", 8)
    max_futures = limits.get("max_futures_positions", 4)
    max_total = limits.get("max_total_positions", 15)
    ...
    futures_count = sum(1 for p in open_positions if _is_futures_position(p))
    total_count = forex_count + stock_count + futures_count

    if total_count >= max_total:
        raise PortfolioRiskError(
            f"Max total positions reached ({total_count}/{max_total})"
        )
    ...
    if instrument_type == "futures" and futures_count >= max_futures:
        raise PortfolioRiskError(
            f"Max futures positions reached ({futures_count}/{max_futures})"
        )
...
def _is_futures_position(pos: dict) -> bool:
    """Check if a position dict represents a futures position."""
    sym = pos.get("symbol", "")
    return sym.upper() in _FUTURES_SYMBOLS
