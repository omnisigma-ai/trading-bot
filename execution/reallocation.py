...
class ProfitReallocator:
    ...
    def _execute_value_stock_purchase(
        ...
        # Fetch fundamentals — use extended fetcher if snowflake enabled
        snowflake_cfg = cfg.get("snowflake", {})
        use_snowflake = snowflake_cfg.get("enabled", False)

        if use_snowflake:
            from data.asx_extended_fundamentals import fetch_extended_universe
            fundamentals = fetch_extended_universe(
                symbols=symbols,
                logger=self.logger,
                max_age_hours=cfg.get("cache_hours", 24),
            )
            snowflake_weight = snowflake_cfg.get("weight", 0.20)
            dcf_config = snowflake_cfg.get("dcf", {})
        else:
        ...
        # Score universe (with snowflake blending if enabled)
        weights = cfg.get("weights")
        scores = score_universe(
            fundamentals, sector_map, weights,
            snowflake_weight=snowflake_weight,
            snowflake_dcf_config=dcf_config,
        )
        ...
        # Select best stock
        min_snowflake = snowflake_cfg.get("min_snowflake_score", 0) if use_snowflake else 0
        best = select_best_stock(
            scores, current_holdings, max_position, min_score, min_moat,
            min_snowflake=min_snowflake,
        )
        ...
        try:
            for s in scores:
                context = {
                    ...
                    "roe": s.roe,
                    "snowflake_total": s.snowflake_total,
                    "snowflake_attempted": s.snowflake_attempted,
                    "dcf_margin_of_safety": s.dcf_margin_of_safety,
                }
                if s.selected:
                    decision = "accept"
