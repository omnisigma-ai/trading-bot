...
Provides the feedback loop:
  ...
  Entry fills   → notify "ORDER FILLED", update DB
...
The IB connection stays alive after order placement via ib.sleep(),
which runs the event loop while waiting.
"""
...
class TradeMonitor:
    ...
    def stop(self) -> None:
        ...
        # Include no-signal pairs already in results
    ...
    def _on_exec_details(self, trade: Trade, fill: Fill) -> None:
        ...
        pair = trade.contract.symbol + trade.contract.currency if trade.contract else "UNKNOWN"
        ...
        if order_id in self._entry_ids:
            ...
            print(f"[Monitor] Entry filled: {group.pair} {side} @ {fill_price}")
        ...
        # TP filled → trade won
            ...
            self.logger.log_trade_closed(db_id, fill_price, "TP", pips, pnl)
            ...
            print(f"[Monitor] TP hit: {group.pair} {side} @ {fill_price} | +{pips:.1f} pips | +${pnl:.2f}")
        ...
        # SL filled → trade lost
            ...
            pips, pnl = self._calc_pnl(side, signal.entry, fill_price, group.pair, group.lot_size)
            self.logger.log_trade_closed(db_id, fill_price, "SL", abs(pips), pnl)
            ...
            print(f"[Monitor] SL hit: {group.pair} {side} @ {fill_price} | {pips:.1f} pips | ${pnl:.2f}")
